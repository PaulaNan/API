...
class TestBooks:
    ...
    def test_get_all_books_type_and_limit(self):
        ...
        assert len(r.json()) == 2, 'type &limit is not working'
        assert r.json()[0]['type'] == 'fiction', 'type filter is not working'
        assert r.json()[1]['type'] == 'fiction', 'type filter is not working'
        # assert r.json()[2]['type'] == 'fiction', 'type filter is not working'
    ...
    def test_get_book_invalid_id(self):
        ...
        assert r.json()['error'] == 'No book with id 212', 'invalid id message is not ok'
